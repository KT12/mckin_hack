...
def feat_eng_train(fn='train_ZoGVYWq.csv'):
    ...
    nan_var = ['late_3_6m', 'late_6_12m', 'late_12m', 'app_score', 'premium_differential', 'count_premiums_paid', 'age']
    ...
    X['premium_differential'] = \
        (X['count_premiums_paid'] /(X['age'] -20)) - X['late_3_6m'] - (1/2)* X['late_6_12m'] - (1/4) * X['late_12m']
...
def feat_eng_test(fn, rescale_dict):
    # Load X set
    X = pd.read_csv(fn)
    # Rename vars
    X.rename(columns={
    ...
    X['late_3_6m'].fillna(rescale_dict['late_3_6m_med'], inplace=True)
    X['late_6_12m'].fillna(rescale_dict['late_6_12m_med'], inplace=True)
    X['late_12m'].fillna(rescale_dict['late_12m_med'], inplace=True)
    X['app_score'].fillna(rescale_dict['app_score_med'], inplace=True)
...
def main():
    ...
    for k in X.columns:
        ...
        print(X[k].max())
    best_rf_clf =  rf(X, y)
    X_data, y_ids = feat_eng_test('test_66516Ee.csv', rescale_dict)
    # Compute ROC_AUC
